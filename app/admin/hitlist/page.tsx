...
const DraggableList = dynamic(() => import("@/components/admin/hitlist/draggable-list"), { 
  ssr: false,
  loading: () => <div className="p-4 text-gray-500 text-center text-sm animate-pulse flex items-center justify-center h-full">Loading list...</div>
})
...
export default function AddSongsClientComponent() {
  ...
  const handleDeleteFuture = useCallback((id: number) => {
    handleDelete(id, 'future')
  }, [handleDelete])
            ...
            <div className="overflow-y-auto flex-1 p-1 custom-scrollbar">
              {futureSongs.length === 0 ? (
                ...
                <DraggableList 
                  ...
                  handleDelete={handleDeleteFuture}
                />
              )}
            </div>
          </div>
