...
function HitlistPlayerComponent({ activeSong }: { activeSong?: Song }) {
  ...
  const trackId = useMemo(() => 
    activeSong?.spotify_link ? getTrackId(activeSong.spotify_link) : "", 
  [activeSong?.spotify_link])

  const bgStyle = useMemo(() => 
    activeSong?.image_url ? { backgroundImage: `url(${activeSong.image_url})` } : {}, 
    ...
    <div className="h-full w-full relative overflow-hidden rounded-3xl shadow-sm border border-gray-100 dark:border-white/10 flex flex-col items-center justify-center bg-black">
      {activeSong?.image_url && (
        <div className="absolute inset-0 z-0">
           ...
           <div 
             className="absolute inset-0 blur-xs scale-125 opacity-80 bg-cover bg-center"
             style={bgStyle} 
           />
        </div>
      )}
      ...
      <div className="relative z-20 w-full max-w-md px-4"> 
        {activeSong?.spotify_link && trackId ? (
          <iframe
            ...
            loading="lazy"
            className="shadow-2xl rounded-xl mx-auto w-70.5 lg:w-88 border-none" 
          />
        ) : (
...
export const HitlistPlayer = memo(HitlistPlayerComponent)